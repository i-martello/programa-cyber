...
const PcList = ({name, precioInicial, precioPorHora}: any) => {
  ...
  return (
    <li className="w-full text-[30px] flex cursor-default border-b-2 border-neutral-800 border-opacity-100 py-6  dark:border-opacity-50">
      ...
      <div>{ active ?
        <Cronometro isActive={setActive} precioInicial={precioInicial} precioPorHora={precioPorHora} /> : <div className="mx-5">cerrado</div>
        }</div>
    </li>
  );
};
